...
# get or create role
def generate_policy_string(region, account_id, lambda_name):
    policy = {
        "Version": "2012-10-17",
        "Statement": [
            {
...
policy_document = generate_policy_string(args.aws_region, aws_account_id, args.lambda_function_name)
...
def get_or_create_lamda_function(session, function_name, role_arn, path_to_code_zip, pingsafe_api_key):
...
webhook_url, api_key = get_or_create_lamda_function(session, args.lambda_function_name, role_arn, 'src/deployment-package.zip', pingsafe_api_key)
